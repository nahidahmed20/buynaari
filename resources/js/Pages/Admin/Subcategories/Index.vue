...
// Props from Laravel
const subcategories = ref(usePage().props.subcategories || [])
...
const totalPages = computed(() => Math.ceil(filteredSubcategories.value.length / itemsPerPage.value))
...
const parseKeywords = (metaKeywords) => {
  ...
  try {
    if (typeof metaKeywords === "string" && metaKeywords.startsWith("[")) return JSON.parse(metaKeywords)
    if (typeof metaKeywords === "string") return metaKeywords.split(",").map(tag => tag.trim())
    ...
    <div class="container">
      ...
      <div class="row align-items-center mb-3 g-2">
        ...
        <div class="col-12 col-md-3">
          <input v-model="search" type="text" class="form-control" placeholder="🔍 Search subcategories..." />
        </div>
        <div class="col-12 col-md-3 d-flex justify-content-md-end justify-content-center">
          <Link :href="route('subcategories.create')" class="btn btn-gradient btn-sm text-white w-100 w-md-auto"
          ...
          <tbody>
            <tr v-for="(sub, index) in paginatedSubcategories" :key="sub.id">
              ...
              <td class="text-center">
                <img :src="`/storage/${sub.image}`" alt="Subcategory Image" height="80" width="80" class="rounded" />
              </td>
              <td class="text-center">
                <span v-for="(tag, i) in parseKeywords(sub.meta_keywords)" :key="i" class="badge bg-success me-1">
                  {{ tag }}
                </span>
              </td>
              <td class="text-center">
                <div class="d-flex justify-content-center gap-2">
                  <button class="btn btn-sm btn-info text-white" @click="showSubCategory(sub)">👁 Show</button>
                  <Link :href="route('subcategories.edit', sub.id)" class="btn btn-sm btn-warning text-white">✏ Edit</Link>
                  <button class="btn btn-sm btn-danger text-white" @click="deleteSubCategory(sub.id)">🗑 Delete</button>
                </div>
              </td>
            </tr>
            <tr v-if="paginatedSubcategories.length === 0">
              <td colspan="8" class="text-center text-muted py-3">No subcategories found.</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="modal fade" :class="{ show: showModal }" style="display: block;" v-if="showModal">
        <div class="modal-dialog modal-lg modal-dialog-centered">
          <div class="modal-content shadow-lg border-0 rounded-4" style="overflow: hidden;">
            <div class="modal-header text-white" style="background: linear-gradient(135deg, #ff7e5f, #feb47b);">
              <h5 class="modal-title fw-bold"><i class="bi bi-grid-fill me-2"></i> Subcategory Details</h5>
              <button type="button" class="btn-close btn-close-white" @click="closeModal"></button>
            </div>
            <div class="modal-body p-4" style="background: linear-gradient(180deg, #ffffff, #f7f9fc);">
              <div class="row g-3">
                <div class="col-md-6">
                ...
                <div v-if="selectedSubCategory.image" class="col-12 text-center mt-3">
                  ...
                  <img :src="`/storage/${selectedSubCategory.image}`" alt="Subcategory Image" class="img-thumbnail shadow-lg"
                       style="width: 200px; height: 200px; object-fit: cover; border-radius: 12px;" />
                </div>
              </div>
            </div>
            <div class="modal-footer" style="background: #fafafa;">
              <button class="btn btn-primary border me-2" @click="closeModal"><i class="bi bi-x-circle me-1"></i> Close</button>
            </div>
          </div>
        </div>
      </div>
